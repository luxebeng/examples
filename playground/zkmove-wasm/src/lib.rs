...
#[wasm_bindgen]
pub fn run_program(_code_frontend: &str, _inputs_frontend: &str) -> Result<Outputs, JsValue> {
...
#[wasm_bindgen]
pub fn prove_program(_code_frontend: &str, _inputs_frontend: &str) -> Result<Outputs, JsValue> {
    let mut vmcircuit = VMCircuitWasm::new();
    vmcircuit.setup_circuit();
    let _ = vmcircuit.mock();
    vmcircuit.gen_vk();
...
pub fn verify_program(
    _code_frontend: &str,
    _inputs_frontend: &str,
    _outputs_frontend: &str,
    _proof: Vec<u8>,
) -> Result<u32, JsValue> {
    let result = 1;
    Ok(result)
